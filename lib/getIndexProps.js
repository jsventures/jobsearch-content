...
const HOME_MD = `
We’re both senior engineers who worked together at Facebook and Airbnb. A bit back, we went on a 100-day long job search, completed a combined 20 onsites, and landed senior/staff offers.

Throughout the process, we spoke to each other every day and shared what we learned.
We discovered that though there was a lot of great information online on interview prep — from cracking algorithm interviews to negotiating compensation. However, the concepts that made the biggest difference in our job search weren’t out there.

These were concepts like signal & scope — two concepts you need to communicate your level. We saw a lot of information about offer negotiations, but rarely did they include discussing scope, which makes the biggest difference for large companies. There was a lot information on algorithm interviews, but rarely information on system design and experience interviews. There was a lot of conversations about referrals, but rarely did people talk about how to build a team of mentors, referrals, and peers for your job search.
...
So, we decided to make this course. We recruited our friends Luba and Aamir, and even made videos. 

We initially planned to charge for this, but decided that it would be best to expand the pie knowledge instead. Hence, we're proud to bring you the whole of jobsearch.dev, for free. We hope you enjoy this, and it helps you on your journey.

[Joe](https://joeaverbukh.com) & [Stopa](https://stopa.io)
...
export default function getAllContent() {
  const content = CONTENT.map((items) =>
    items.map(([navTitle, p, videoSlug]) => {
      ...
      return {
        slug: `nav-${navTitle.split(" ").join("-").toLocaleLowerCase()}`,
        navTitle,
        chapterTitle,
        html: marked(md),
